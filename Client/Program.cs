...
namespace Client
{
    ...
    class Program
    {
        ...
        static readonly BigInteger clientPrivateKey = 4;
        ...
        {
            Console.WriteLine("=========================================");
            Console.WriteLine("   SECURE CLIENT (TCP | DH + AES + SHA256) ");
            Console.WriteLine("=========================================\n");
            ...
            BigInteger A = BigInteger.ModPow(g, clientPrivateKey, p);

            SecurePacket dhPacketTx = new SecurePacket { Type = MessageType.DH_PubKey, PublicKey = A.ToString() };
            Console.ForegroundColor = ConsoleColor.Cyan;
            Console.WriteLine($"[TX] Sending Client Public Key (A): {A}");
            Console.ResetColor();
            await writer.WriteLineAsync(JsonSerializer.Serialize(dhPacketTx));

            string? serverMsg = await reader.ReadLineAsync();
            SecurePacket dhPacketRx = JsonSerializer.Deserialize<SecurePacket>(serverMsg!)!;
            BigInteger B = BigInteger.Parse(dhPacketRx.PublicKey);
            Console.ForegroundColor = ConsoleColor.Green;
            Console.WriteLine($"    <- [RX] Server Public Key (B) received: {B}");

            BigInteger S = BigInteger.ModPow(B, clientPrivateKey, p);
